    ...
    def build_dict(self):
        ...
        for name in enzymedict:
            ...
            if typename in typedict:
                ...
                tydct = dict([(k,v) for k,v in list(tydct.items()) if k in commonattr])
                enzlst.append(name)
        ...
        for name in sorted(classdict) :
            ...
            for key, value in list(classdict[name].items()) :
                results.write("        %s : %s,\n" % (repr(key), repr(value)))
            results.write("    }\n")
            results.write("rest_dict[%s] = _temp()\n" % repr(name))
            results.write("\n")
        print('OK.\n')
        print('Writing the dictionary containing the suppliers datas.\t\t', end=' ')
