...
class SeqRecordCreation(unittest.TestCase):
    ...
    def test_annotations(self):
        ...
        rec = SeqRecord(
            Seq("ACGT", Alphabets.DNA), id="Test", name="Test", description="Test"
        )
    ...
    def test_replacing_seq(self):
        ...
        try:
            rec.seq = Seq("ACGTACGT", Alphabets.DNA)
            self.fail(
        ...
        rec.seq = Seq("NNNN", Alphabets.DNA)
        self.assertEqual(str(rec.seq), "NNNN")
        self.assertEqual(rec.letter_annotations, {"example": [1, 2, 3, 4]})

    def test_valid_id(self):
        with self.assertRaises(TypeError):
            SeqRecord(Seq("ACGT", Alphabets.DNA), id={})

    def test_valid_name(self):
        with self.assertRaises(TypeError):
            SeqRecord(Seq("ACGT", Alphabets.DNA), name={})

    def test_valid_description(self):
        with self.assertRaises(TypeError):
            SeqRecord(Seq("ACGT", Alphabets.DNA), description={})

    def test_valid_dbxrefs(self):
        with self.assertRaises(TypeError):
            SeqRecord(Seq("ACGT", Alphabets.DNA), dbxrefs={})

    def test_valid_annotations(self):
        with self.assertRaises(TypeError):
            SeqRecord(Seq("ACGT", Alphabets.DNA), annotations=[])

    def test_valid_features(self):
        with self.assertRaises(TypeError):
            SeqRecord(Seq("ACGT", Alphabets.DNA), features={})
...
class SeqRecordMethods(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.record = SeqRecord(
            Seq("ABCDEFGHIJKLMNOPQRSTUVWZYX", Alphabets.Protein),
            id="TestID",
            name="TestName",
    ...
    def test_contains(self):
        self.assertIn(Seq("ABC", Alphabets.Protein), self.record)
...
Seq('ABCDEFGHIJKLMNOPQRSTUVWZYX', Alphabets.Protein)"""
    ...
    def test_repr(self):
        expected = (
            "SeqRecord(seq=Seq('ABCDEFGHIJKLMNOPQRSTUVWZYX', Alphabets.Protein), "
