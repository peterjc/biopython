...
if False:
    ...
    records = list(SffIterator(open("Roche/E3MFGYR02_random_10_reads.sff", "rb")))
    ...
    out_handle.close()
    records2 = list(SffIterator(open("Roche/E3MFGYR02_alt_index_at_start.sff", "rb")))
    for old, new in zip(records, records2):
        assert str(old.seq) == str(new.seq)
    i = list(_sff_do_slow_index(open("Roche/E3MFGYR02_alt_index_at_start.sff", "rb")))
    ...
    w.write_header()  # this time with index info
    out_handle.close()
    records2 = list(SffIterator(open("Roche/E3MFGYR02_alt_index_in_middle.sff", "rb")))
    for old, new in zip(records, records2):
        assert str(old.seq) == str(new.seq)
    j = list(_sff_do_slow_index(open("Roche/E3MFGYR02_alt_index_in_middle.sff", "rb")))
    ...
    with open("Roche/E3MFGYR02_alt_index_at_end.sff", "w") as out_handle:
        ...
        out_handle.seek(0)
        w.write_header()  # this time with index info
    records2 = list(SffIterator(open("Roche/E3MFGYR02_alt_index_at_end.sff", "rb")))
    for old, new in zip(records, records2):
        assert str(old.seq) == str(new.seq)
    with unittest.TestCase.assertRaises(None, ValueError):
        print(ReadRocheXmlManifest(open("Roche/E3MFGYR02_alt_index_at_end.sff", "rb")))
    k = list(_sff_do_slow_index(open("Roche/E3MFGYR02_alt_index_at_end.sff", "rb")))
    print("Done")
