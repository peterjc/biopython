...
class ModTest(unittest.TestCase):
    ...
    def testAlignmentFileIsValid(self):
        self.assertRaises(
            (AttributeError, TypeError, OSError), yn00.Yn00, alignment=list()
        )
    ...
    def testCtlFileValidOnRun(self):
        ...
        self.yn00.out_file = self.out_file
        self.assertRaises(
            (AttributeError, TypeError, OSError), self.yn00.run, ctl_file=list()
        )
    ...
    def testCtlFileValidOnRead(self):
        self.assertRaises(
            (AttributeError, TypeError, OSError), self.yn00.read_ctl_file, list()
        )
        self.assertRaises(
    ...
    def testResultsValid(self):
        self.assertRaises((AttributeError, TypeError, OSError), yn00.read, list())
    ...
    def testParseLongNames(self):
        ...
        for results_file in glob.glob(pattern):
            ...
            self.assertEqual(
                set([len(v) for taxa in results.values() for v in taxa.values()]),
                set([5]),
            )
